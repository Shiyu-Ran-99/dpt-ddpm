...
def train_ctgan(
    ...
    if lib.load_json(real_data_path / "info.json")["task_type"] != "regression":
        cat_features += ["y"]
    print(f"cat_features is {cat_features}")
    ...
    ctgan.fit(X, discrete_columns=cat_features)
...
def main():
    ...
    ctabgan = train_ctgan(args.parent_dir, args.real_data_path, change_val=True)
    sample_ctgan(ctabgan, args.parent_dir, args.real_data_path, args.train_size, change_val=True)
