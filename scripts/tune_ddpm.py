...
def objective(trial):
    ...
    lr_dp = trial.suggest_loguniform('lr_dp', 0.0001, 0.04)
    ...
    subprocess.run(['python3.9', f'{pipeline}', '--config', f'{exps_path / "config.toml"}', '--train', '--change_val'],
                   check=True)
    ...
    for sample_seed in range(n_datasets):
        ...
        subprocess.run(['python3.9', f'{pipeline}', '--config', f'{exps_path / "config.toml"}', '--sample', '--eval',
                        '--change_val'], check=True)
...
if args.eval_seeds:
    ...
    subprocess.run(
        ['python3.9', f'{eval_seeds}', '--config', f'{best_exp}', '10', "ddpm", eval_type, args.eval_model, '5'],
        check=True)